  ...
  useEffect(() => {
    const fetchTakenRepairs = async () => {
      try {
        ...
        setTakenRepairs(Array.isArray(data) ? data : []);
      } catch (error) {
        console.error("Error fetching taken repairs:", error);
        setTakenRepairs([]); // Reset to an empty array on error
      }
    };
              ...
              {currentRepairs.length > 0 ? (
                currentRepairs.map((repair, index) => (
                  <tr key={repair._id} className="border-b">
                    ...
                    <td className="py-2 px-4">${(repair.serviceFee || 0).toFixed(2)}</td>
                    <td className="py-2 px-4">${(repair.itemCost || 0).toFixed(2)}</td>
                    <td className="py-2 px-4">${(repair.itemProfit || 0).toFixed(2)}</td>
