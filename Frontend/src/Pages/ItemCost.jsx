...
const ItemCost = () => {
  ...
  useEffect(() => {
    const fetchItemCostData = async () => {
      try {
        const response = await axios.get('http://localhost:3000/api/itemCost');
        if (response.data) {
          ...
          {showSummary
            ? (
              <tr className="hover:bg-[#F68E30] transition duration-150">
                ...
                <td className="py-3 px-5 border-b">${itemCostData.reduce((acc, item) => acc + item.itemCost, 0).toFixed(2)}</td>
              </tr>
            ) : (
              itemCostData.map((entry) => (
                <tr key={entry._id} className="hover:bg-[#F68E30] transition duration-150">
                  <td className="py-3 px-5 border-b">{entry.name}</td> {/* Adjusted field */}
                  <td className="py-3 px-5 border-b">{entry.problem}</td> {/* Adjusted field */}
                  <td className="py-3 px-5 border-b">${entry.itemCost.toFixed(2)}</td>
                </tr>
              ))
            )}
        </tbody>
