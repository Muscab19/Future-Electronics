...
const AllCustomers = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:3000/api/allCustomers') // Adjust to your actual API endpoint
  ...
  const deleteCustomer = (customerIndex, customerId) => {
    axios.delete(`http://localhost:3000/api/allCustomers/${customerId}`)
      .then(() => {
          ...
          <table className="min-w-full border-collapse">
            <thead>
              <tr className="border-b">
                ...
                <th className="py-4 px-3 text-left text-lg">Problem</th>
                <th className="py-4 px-3 text-left text-lg">Status</th>
                <th className="py-4 px-3 text-left text-lg">Technician</th>
                <th className="py-4 px-3 text-left text-lg">Service Fee</th>
                <th className="py-4 px-3 text-left text-lg">Actions</th>
              </tr>
            </thead>
            <tbody>
              {currentCustomers.length > 0 ? (
                currentCustomers.map((customer, index) => (
                  <tr key={customer.id} className="border-b">
                    ...
                    <td className="py-2 px-2 text-base">{customer.problem}</td>
                    <td className="py-2 px-2 text-base">{customer.status}</td>
                    <td className="py-2 px-2 text-base">{customer.technician}</td>
                    <td className="py-2 px-10 text-base">${customer.serviceFee}</td>
                    <td className="py-3 px-10 flex space-x-3">
                      {/* Delete Button */}
                      <button
